  ...
  try {
    ...
    if (jobId && !forceRefresh) {
      try {
        const cached = sqlite.prepare(
          'SELECT result_json, created_at FROM company_analyses WHERE job_id = ? ORDER BY created_at DESC LIMIT 1'
        ).get(jobId) as any;
    ...
    try {
      ...
      if (jobId) {
        try {
          sqlite.prepare(`
            INSERT OR REPLACE INTO company_analyses (job_id, result_json, created_at)
            VALUES (?, ?, ?)
          `).run(jobId, JSON.stringify(result), Date.now());
        } catch (error) {
          console.error('Failed to cache company analysis:', error);
