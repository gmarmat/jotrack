...
// v2.9: Added Claude Sonnet 4.5 (latest)
...
export async function getAiProviderConfig(): Promise<AiProviderConfig | null> {
  ...
  if (settings.provider === 'claude') {
    ...
    model = settings.claudeModel || settings.model || 'claude-3-sonnet-20240229'; // Default to Sonnet 3.5
  } else {
...
/**
/**
 * v1.4: Simplified validation - trust persisted settings, no auto-recovery API calls
 * Only validates model is in allowlist. If invalid, user must update Settings.
 */
function validateModel(model: string): string {
  if (ALLOWED_MODELS.includes(model)) {
    return model;  // ✅ Valid, use it directly
  }
  
  throw new Error(
    `❌ Model "${model}" not supported.\n` +
    `Allowed: ${ALLOWED_MODELS.join(', ')}\n` +
    `Fix: Go to Settings → AI Configuration to select a valid model`
  );
}
...
export async function callAiProvider(
  ...
  // v1.4: Simplified validation - trust settings, no API calls
  ...
  if (config.provider === 'claude') {
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: config.model, // Use only settings, no fallback
...
function buildPromptVariables(capability: string, inputs: any): any {
  ...
  switch (capability) {
    ...
    case 'suggest-answer':
      return {
        question: inputs.question || 'Unknown question',
        resumeContext: inputs.resumeContext || 'No resume context available',
        jdContext: inputs.jdContext || 'No job description context available',
        companyName: inputs.companyName || 'Unknown Company',
        roleTitle: inputs.roleTitle || 'Unknown Role',
        currentAnswer: inputs.currentAnswer || 'No current answer'
      };
    ...
    case 'answer-scoring':
      return {
        ...
        writingStyleProfile: inputs.writingStyleProfile || '{}',
        // Flatten interviewer profile properties for template replacement
        interviewerProfileName: inputs.interviewerProfileName || inputs.interviewerProfile?.name || '',
        interviewerProfileRole: inputs.interviewerProfileRole || inputs.interviewerProfile?.role || '',
        interviewerProfileCommunicationStyle: inputs.interviewerProfileCommunicationStyle || inputs.interviewerProfile?.communicationStyle || '',
        interviewerProfileKeyPriorities: inputs.interviewerProfileKeyPriorities || inputs.interviewerProfile?.keyPriorities || '',
        interviewerProfileRedFlags: inputs.interviewerProfileRedFlags || inputs.interviewerProfile?.redFlags || ''
      };
